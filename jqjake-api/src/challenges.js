...
const challengesWithoutId = [
    ...
    {
        challengeIntroduction: "String interpolation",
        challengeJson: {
            henchPeople: [
                { person: "Lou the Locksmith", pctShare: .10 },
                { person: "Sneaky Steve", pctShare: .5 },
                { person: "Lookout Larry", pctShare: .5 },
            ],
        },
        challengeAnswer: '[.henchPeople[] | "\\(.person) gets a \\(.pctShare * 100) percent share"]',
        challengeInstruction:
            '<div class=\'instruction-content\'></p>String interpolation allows for the construction and modification of string values.#0#For example, given the above JSON, a filter of <c>"The fruit name is \\(.apple)"</c> will output <o>The fruit name is apple</o>. Note that in order to include the value from the source JSON the key must be surrounded by parenthesis and escaped with a forward slash. <p/>The contents of the parenthesis will be evaluated, meaning this is not strictly limited to exact replacement. This can be demonstrated with a filter that performs a basic subtraction operation, like  <c> "If I eat one I will have \\(.quantity - 1) remaining"</c>. jq will subtract 1 from the value of the quantity key and substitute the difference within the parenthesis, the result of which will be <o>If I eat one I will have 2 remaining</o>.<p/>Below is an example of how to use string interpolation with an array. #1#<p/>A filter of <c>[.[] | "\\(.)s are tasty"]</c> will return: #2#<p/>Let\'s break down how this works:<ul><li/><c>.[] |</c> iterates over the elements in the array and passes them to the next part of the filter.<li/><c>"\\(.)s are tasty"</c> appends <b>"s are tasty"</b> to the input through string interpolation.<li/>Lastly that\'s all wrapped in a pair of brackets to return an array.</ul><h><p/>The proceeds from the sale of the remaining pieces will be split according to a predetermined allocation. Jake would like to know how much a few are getting.<p/>Construct an array that meets the following criteria:<ul><li/>The array should contain as many elements as there are objects in the <b>henchPeople</b> array.<li/>Each element in the array should consist of a string that takes the format of "<b>person</b> gets a <b>pctShare</b> percent share".<li/>The value of <b>person</b> should come from the <b>person</b> key.<li/>The value of <b>pctShare</b> should be the product of <b>pctShare</b> multiplied by <b>100</b>.<ul></h></div>',
        challengeInstructionJson: [
            {
                output: { fruitName: "apple", quantity: 3 },
            },
            {
                output: ["plum", "kiwi", "pear"],
            },
            {
                output: ["plums are tasty", "kiwis are tasty", "pears are tasty"],
            },
        ],
    },
